...
function check_cmd_param(cmdCode)
    ...
    if cmdCode == '#REV ' or cmdCode == '#FWD ' then
        ...
        if cmdArgs == nil or cmdArgs == '' then
            ...
            Trace('last_cmd_arg: ' .. last_cmd_arg)
...
function service_run()
    ...
    if err ~= nil then
        ...
        -- login process start
        if string.find(rdata,'connected') == 1 then
            Trace('oppo-rpi connected')
            ...
            while ( tostring(stopflag) ~= 'true' ) do
                ...
                while rcount < rmax do
                    ...
                    if errCode == 0 and string.len(rdata) > 0 then
                        if string.sub(rdata,string.len(rdata)-1,string.len(rdata)) == '\r\n' then
                            break
                        else
    ...
    -- socket disconnect
    bytes, errCode = socket:write( 'logout\r\n' )
    Trace('logout cmd sent', _INFO)
    socket:disconnect()
    socket = nil
